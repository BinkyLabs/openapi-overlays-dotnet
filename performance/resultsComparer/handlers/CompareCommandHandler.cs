...
internal sealed class CompareCommandHandler : AsynchronousCommandLineAction
{
    ...
    {
        var existingBenchmark = await GetBenchmarksAllocatedBytes(existingReportPath, cancellationToken).ConfigureAwait(false);
        ...
        var newBenchmark = await GetBenchmarksAllocatedBytes(newReportPath, cancellationToken).ConfigureAwait(false);
    ...
    private static async Task<Dictionary<string, BenchmarkMemory>?> GetBenchmarksAllocatedBytes(string targetPath, CancellationToken cancellationToken = default)
    {
        ...
        var report = await JsonSerializer.DeserializeAsync(stream, BenchmarkSourceGenerationContext.Default.BenchmarkReport, cancellationToken: cancellationToken).ConfigureAwait(false)
            ?? throw new InvalidOperationException($"Failed to deserialize {targetPath}.");
        return report.Benchmarks
            .Where(static x => x.Memory is not null && x.Method is not null)
